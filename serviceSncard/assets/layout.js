(() => {
  ...
  const links = [
    ...
    ['equipment', 'Оборудование', 'equipment.html'],
    ['cards', 'Изготовление карт', 'cards.html'],
  ];
  const header = document.querySelector('[data-site-header]');
  if (header) header.innerHTML = `<div class="container header-row">
    <a class="brand" href="index.html" aria-label="Сибнефтекарт-Сервис — главная"><img class="brand-logo" src="../zaoSncard/assets/logo2.png" alt="Сибнефтекарт" width="480" height="91"></a>
    <nav class="nav" id="site-nav" aria-label="Навигация Сибнефтекарт-Сервис">${links.map(([id, title, href]) => `<a href="${href}" ${id === page ? 'class="is-active" aria-current="page"' : ''}>${title}</a>`).join('')}<a href="../index.html">Головной сайт</a></nav>
  ...
  if (footer) footer.innerHTML = `<div class="container footer-main">
    ...
    <div class="footer-col"><h2>Разделы</h2><div class="footer-links">${links.map(([, title, href]) => `<a href="${href}">${title}</a>`).join('')}<a href="../index.html">Головной сайт</a></div></div>
